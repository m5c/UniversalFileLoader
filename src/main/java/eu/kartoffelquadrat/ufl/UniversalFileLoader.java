...
public class UniversalFileLoader {
    ...
    /**
     * Helper method to build the corresponding java file (location only, no copying) for a given resource. Also runs a
     * sanity check on the provided resource string.
     *
     * @param resourceName
     * @return File location pointing to a location in the temp / buffer dir. The referenced file may not yet exists on
     * disk.
     */
    ...
    /**
     * Helper method to create an physical file on disk, filled with identical content as a given resource on the
     * classpath. Uses only reliable java resource handling for this task, so it will work no matter how this program
